...
Indexing of the texture array (and colors, flagschems etc.) for the canvas variant is done by writing the index values into vertex attributes, namely into additional UV channels. Some of the texture arrays like one for the banner use multiple color channels to store back and front parts of the shape to achieve layered effect.
